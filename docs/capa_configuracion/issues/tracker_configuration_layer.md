...
| BESS-CONFIG-013 | Coordinate Editors And Preserve Auditable Series History | AFK | ready-for-agent | In Review | BESS-CONFIG-011 | [BESS-CONFIG-013-coordinate-editors-and-preserve-auditable-series-history.md](BESS-CONFIG-013-coordinate-editors-and-preserve-auditable-series-history.md) |
...
| BESS-CONFIG-015 | Resolve Console Blocks With The Correct Engineer Action | AFK | ready-for-agent | In Review | BESS-CONFIG-012, BESS-CONFIG-014 | [BESS-CONFIG-015-resolve-console-blocks-with-the-correct-engineer-action.md](BESS-CONFIG-015-resolve-console-blocks-with-the-correct-engineer-action.md) |
| BESS-CONFIG-016 | Compare Two Configured Console Runs Safely | AFK | ready-for-agent | In Review | BESS-CONFIG-009, BESS-CONFIG-010 | [BESS-CONFIG-016-compare-two-configured-console-runs-safely.md](BESS-CONFIG-016-compare-two-configured-console-runs-safely.md) |
| BESS-CONFIG-017 | Prove The Configuration Layer End To End | AFK | ready-for-agent | In Review | BESS-CONFIG-001 through BESS-CONFIG-016 | [BESS-CONFIG-017-prove-the-configuration-layer-end-to-end.md](BESS-CONFIG-017-prove-the-configuration-layer-end-to-end.md) |
...
| 2026-08-25 | BESS-CONFIG-016 | Todo -> In Review | Added `GET /api/console/{id}/run-comparison?left=&right=` on one shared `console_results_block` helper that run detail now reads too, so both sides cross the same configured allowlist; a public history id resolves only against its own console, so a guessed, foreign, analyst-triggered, draft-console or revoked run is 404; a side without readable results stays `unavailable` with no artifact name or technical reason; and `build_console_run_comparison` differences only the KPIs both sides published, in configured order. The console history gained per-run compare checkboxes, a two-pick gate and a side-by-side panel that never leaves `/console/:id`; `PortalResultsBlock` took an id prefix and an empty-state message so two blocks can share one page. 802 Python tests (7 environment skips), 124 Vitest tests, tsc, eslint, focused Prettier, the regenerated API schema and the production build passed. An isolated Chrome narrative configured a new console 13, ran it at 40 MW and at 12 MW, and compared both: 4792.2 USD against 4075.4 USD, -716.9 USD of difference, distinct series on each side, no browser-console warnings and no internal metadata in the payload. |
| 2026-08-26 | BESS-CONFIG-013 | Todo -> In Progress | Started coordinated leases, auditable history, undo and internal restore under TDD at the confirmed persistence, HTTP and React seams. |
| 2026-08-26 | BESS-CONFIG-013 | In Progress -> In Review | Added all-or-none conditional group leases with heartbeat, expiry, release and admin force release; opaque ETag-protected saves and undo; reduced public operation history; append-only internal revision restore; explicit SQLite/PostgreSQL transaction boundaries; and the operator/admin React surfaces under TDD. 817 Python tests (7 PostgreSQL environment skips), 127 Vitest tests, tsc, eslint on touched files, focused Prettier, the generated API contract and the production build passed. A Chrome narrative confirmed the operator history, edit lease and undo plus the admin holder, force-release and restore controls with no console warnings or errors; the repository-wide Prettier check still reports 27 unrelated baseline files. |
| 2026-08-26 | BESS-CONFIG-015 | Todo -> In Progress | Started the focused engineer recovery workflow under TDD at the confirmed persistence, HTTP and React seams. |
| 2026-08-26 | BESS-CONFIG-015 | In Progress -> In Review | Added typed recovery actions for moved dependencies and exact unavailable parameter or series-column targets; successful validation and corrected saves clear only the attestation they prove; canonical revision drift appears as a non-blocking old-copy badge without mutating operator data; and failed operator runs link internal engineers to the existing technical detail while external access remains 404. 825 Python tests (7 PostgreSQL environment skips), 133 Vitest tests, tsc, eslint, focused Prettier, the generated API contract and the production build passed. An isolated Chrome narrative exercised both recovery paths, preserved the mixed-state block, verified the old-copy badge and opened the technical run detail with no console errors; the visual pass also caught and corrected the badge contrast. |
| 2026-08-27 | BESS-CONFIG-017 | Todo -> In Progress | Started the closing proof under TDD at the confirmed FastAPI, SQLite migration, visible React and built-browser seams. |
| 2026-08-27 | BESS-CONFIG-017 | In Progress -> In Review | Added the nine-test configuration-layer acceptance narrative and reproducible Chrome fixture; refreshed twelve Playwright flows onto external capabilities and configured portal results; fixed same-timestamp capability revocation state; and recorded the complete scope closure. 834 Python tests (7 PostgreSQL environment skips), 133 Vitest tests, 532 Julia tests, schema drift, ESLint, production build, 12 Playwright tests and the Chrome operator/client narrative passed. |
