...
| --- | --- | --- | --- | --- |
| Caso normalizado | `optimization_cases` como fuente editable y `scenario_versions` como snapshot inmutable. | `optimization_cases` implementada y conectada 1:1 con `scenarios` para el editor hidraulico. Campos: `scenario_id`, `case_key`, `display_name`, `validation_payload_json`, auditoria create/update. `validation_payload_json` ahora guarda snapshots exitosos `hydraulic_v3_preview` con `validation_hash`, `system_case`, `julia_validation` y estado `stale`. | Conectar promocion v3 contra el caso normalizado en issues posteriores. | BESS-HYDRO-DIAGRAM-005 |
...
- `AnalystStore.generate_hydraulic_v3_preview` arma el contrato
  ...
  requerimientos `natural_inflow_m3s`; `persist_hydraulic_v3_validation`
