...
| BESS-TS4-006 | Make Result Indexing Idempotent And Failure-Safe | AFK | ready-for-agent | Done | 2026-07-21 | 2026-07-22 | BESS-TS4-001 | [BESS-TS4-006-make-result-indexing-idempotent-and-failure-safe.md](BESS-TS4-006-make-result-indexing-idempotent-and-failure-safe.md) |
| BESS-TS4-007 | Serve Run Tables And Charts From BBDD With Artifact Fallback | AFK | ready-for-agent | Done | 2026-07-23 | 2026-07-24 | BESS-TS4-002, BESS-TS4-003, BESS-TS4-004 | [BESS-TS4-007-serve-run-tables-and-charts-from-bbdd-with-artifact-fallback.md](BESS-TS4-007-serve-run-tables-and-charts-from-bbdd-with-artifact-fallback.md) |
| BESS-TS4-008 | Rebuild BBDD Results From Artifacts For Historical Runs | AFK | ready-for-agent | Done | 2026-07-27 | 2026-07-27 | BESS-TS4-005, BESS-TS4-006 | [BESS-TS4-008-rebuild-bbdd-results-from-artifacts-for-historical-runs.md](BESS-TS4-008-rebuild-bbdd-results-from-artifacts-for-historical-runs.md) |
| BESS-TS4-009 | Compare Two Runs Of The Same Case | AFK | ready-for-agent | Done | 2026-07-08 | 2026-07-08 | BESS-TS4-002, BESS-TS4-004 | [BESS-TS4-009-compare-two-runs-of-the-same-case.md](BESS-TS4-009-compare-two-runs-of-the-same-case.md) |
| BESS-TS4-010 | Finalize TS-4 Acceptance Suite And Docs | AFK | ready-for-agent | Done | 2026-07-30 | 2026-07-30 | BESS-TS4-001 through BESS-TS4-009 | [BESS-TS4-010-finalize-ts4-acceptance-suite-and-docs.md](BESS-TS4-010-finalize-ts4-acceptance-suite-and-docs.md) |
...
| 2026-07-08 | BESS-TS4-006 | Done | Added a hardened `index_run_results` orchestrator (per-surface isolation, logging, never raises) replacing the silent `try/except: pass` hook in `runner.py`, and cleanup-on-exception in the three `replace_run_*_result_index` writers so a failed write always converges to either a complete index or none at all (never a corrupt partial one), even under PostgreSQL's autocommit mode. Verified against real Postgres with a forced duplicate-timestamp partial-failure-then-retry, and live in Chrome with a fresh real Julia run (Run 26) indexing and rendering correctly. |
| 2026-07-08 | BESS-TS4-007 | Done | Confirmed the BBDD-first/artifact-fallback read path already covered every run-results surface (dispatch, asset dispatch, summary, charts, dashboards, publications) as a side effect of TS4-001/003/004, since `read_run_results` resolves each surface independently and every consumer already passes `store=`. Added the missing test coverage for the partially-indexed/mixed-source contract (`MixedSurfaceReadPathTests` in `tests/test_ts4_result_indexing.py`, plus a dashboard-template mixed-source regression test), and verified live against real Postgres and a real Julia run: Run 26 (fully indexed) rendered identically with `dispatch.csv`/`summary.json` temporarily removed from disk, and Run 8 (a pre-TS4, never-indexed historical run) still rendered fully from artifacts with no regression. |
| 2026-07-08 | BESS-TS4-008 | Done | Added `rebuild_run_results`/`rebuild_all_run_results` in `app/result_indexing.py`, reusing `index_run_results` (same parsing/normalization/lineage/write path as live indexing) with a `run_results_fully_indexed` skip-unless-forced idempotency check, plus `AnalystStore.list_succeeded_runs()` and two admin endpoints (`POST /api/admin/runs/{run_id}/rebuild-results`, `POST /api/admin/runs/rebuild-results`, both admin-gated). Verified against real Postgres and live in Chrome: rebuilt historical Run 8 via the admin endpoint, confirmed the run results page rendered identically with `dispatch.csv`/`summary.json`/`asset_dispatch.csv` temporarily removed from disk (BBDD-first proof), then ran the bulk endpoint twice against all 18 real succeeded runs — first pass indexed 11 never-indexed legacy runs (including pre-TS3 runs with absent variant/date-range lineage) and skipped the 4 already-indexed ones, second pass converged to 0 indexed / 15 skipped, with 3 genuinely orphaned runs (missing artifact files on disk) correctly and stably reported under `failed` both times. |
| 2026-07-08 | BESS-TS4-009 | Done | Added `app/result_comparison.py` (`compare_runs`, `ComparisonError`) sourcing exclusively from BBDD result indexes: same-case check via `get_run_lineage`, `run_results_fully_indexed` pre-check with a `ComparisonError` pointing at the `rebuild-results` admin endpoint, scalar summary-KPI diffing, and period-level diffing for a selected `signal_key` matched by timestamp (so non-overlapping date ranges surface `null` on the side without data instead of raising). Wired `GET /api/run-comparisons?baseline_run_id&candidate_run_id&series`. Added `RunComparisonView` in `frontend/src/Workspace.tsx` (run pickers scoped to the scenario, side-by-side variant/date-range context, KPI table, series selector, period-diff table) reachable from a new "Comparar corridas" link on the scenario page, routed at `/scenarios/:scenarioId/runs/compare`. `tests/test_ts4_run_comparison.py` (10 tests) covers KPI+context diffing, period-level diffing including non-overlapping ranges, graceful failure for non-indexed and cross-case runs, differing variants/ranges, and the API layer end to end; full backend suite (341 tests) and frontend suite (61 tests, tsc, eslint, `api:generate`/`api:check`, build) stay green. Verified live in Chrome against real Postgres: compared real indexed runs 21 vs 20 of scenario 28 (KPI table, variant/date-range context, and the series selector re-fetching a different signal all rendered correctly), confirmed the empty-state message for a scenario with fewer than two succeeded runs (scenario 34), and confirmed the cross-case graceful error (422, "same case") via an authenticated fetch against the real running server. The non-indexed-run graceful error (409, rebuild-path message) has no real same-case pair to exercise live without mutating production index rows, so it is proven by the 3 dedicated automated tests instead. |

| 2026-07-08 | BESS-TS4-010 | Todo -> Done | Closing proof issue, no production code change needed: TS4-001 through TS4-009 already implement indexed dispatch/asset-dispatch/summary reads, lineage, idempotent re-indexing, rebuild and comparison end to end. Added `tests/test_ts4_acceptance.py` (TDD, tracer bullet first, 2 tests): one continuous story that proves hybrid result indexing, asset rows, summary KPIs, frozen variant/date-range lineage, idempotent re-indexing, legacy artifact fallback, admin rebuild, hydro-only signal coverage, and same-case comparison with KPI and period-level diffs; and a second test asserting README, this issue, this tracker and the new manual checklist are all in their closed state. Added a new README section ("TS-4: Result Series, Rebuild, And Run Comparison") plus `docs/series_tiempo/iter4/pruebas_manuales_ts4.md`. Focused acceptance command: `.\.venv\Scripts\python.exe -m unittest tests.test_ts4_acceptance -v`. Live Chrome verification against the real PostgreSQL-backed app created smoke runs `27` (legacy full-artifact) and `28` (hydro-only), proved `Run 27` renders before rebuild, still renders after rebuild with `dispatch.csv`/`asset_dispatch.csv`/`summary.json` temporarily removed from disk (BBDD-first proof), proved `Run 28` exposes hydro result families and hydro charts, and proved the real comparison page for scenario `28` renders runs `21` vs `20` with baseline/candidate context, KPI diffs and the series selector. This closes TS-4. |
