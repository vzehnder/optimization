...
| BESS-TS3-008 | Run One Case With Two Variants And Preserve Legacy Runs | AFK | ready-for-agent | Done | 2026-07-28 | 2026-07-29 | BESS-TS3-004, BESS-TS3-007 | [BESS-TS3-008-run-one-case-with-two-variants-and-preserve-legacy-runs.md](BESS-TS3-008-run-one-case-with-two-variants-and-preserve-legacy-runs.md) |
| BESS-TS3-009 | Finalize TS-3 Acceptance Suite And Docs | AFK | ready-for-agent | Done | 2026-07-30 | 2026-07-30 | BESS-TS3-001 through BESS-TS3-008 | [BESS-TS3-009-finalize-ts3-acceptance-suite-and-docs.md](BESS-TS3-009-finalize-ts3-acceptance-suite-and-docs.md) |
...
| 2026-07-07 | BESS-TS3-008 | Todo -> Done | Two-variant comparison and legacy-run coexistence proven TDD-first. New backend test (`tests/test_ts3_case_variant_api.py`) runs one case with the default variant bound to one price set and a cloned "Stress prices" variant bound to a different price set over the same range, asserting distinct `scenario_version_id`s, distinct `system_case_json`, distinct series-binding content hashes, and that both runs are individually resolvable to their producing variant's `display_name` via `GET .../runs` + `GET /api/scenario-versions/{id}`; it also creates a legacy (non-variant) `ScenarioVersion` through the pre-TS-3 `POST .../versions` + `POST /api/scenario-versions/{id}/runs` path and confirms it runs unaffected alongside the variant runs. The one real gap this issue surfaced: the case run list (`RunList` in `frontend/src/Workspace.tsx`) rendered every run as just "Version N" with no indication of which input variant produced it, even though `generation_metadata.input_variant.display_name` was already available on every `ScenarioVersion` from TS3-007. Fixed by adding `generation_metadata` to the base `ScenarioVersion` TS type (`frontend/src/api/client.ts`) and having `RunList` append `" | Variante: <display_name>"` when a version's `generation_metadata.kind === "case_input_variant"`, with legacy/no-variant runs falling back to showing no variant segment. Verification: Python suite 293 passed / 2 skipped (up from 292); frontend suite 61 passed (up from 60) plus `tsc -b`, `eslint .`, `api:check`, `build`. Chrome-devtools MCP against real PostgreSQL + real Julia (new project `TS3-008 Chrome QA`, scenario 42, clean grid+battery case with a single combined `price_usd_per_mwh` signal) ran the default variant bound to "Spot A" to Run 17 `succeeded` (HiGHS `OPTIMAL`), cloned it to "Stress prices" rebound to "Spot B" and ran it to Run 18 `succeeded` (HiGHS `OPTIMAL`, distinct set id/hash/topology-parameters snapshot), then created and ran a legacy (non-variant) scenario version to Run 19 `succeeded` (HiGHS `OPTIMAL`); the case run list showed all three together as "Run 19 \| Version 3" (no variant, legacy), "Run 18 \| Version 2 \| Variante: Stress prices", and "Run 17 \| Version 1 \| Variante: Default", with zero console errors. An unrelated pre-existing quirk was found and left unfixed (out of scope): a shared older QA scenario (41) has a grid configured for separate import/export prices, so binding a set that only offers the combined `price_usd_per_mwh` signal there still fails Julia validation; a fresh clean scenario was used instead for this issue's verification. |
| 2026-07-07 | BESS-TS3-009 | Todo -> Done | Closing proof added in `tests/test_ts3_acceptance.py`: one continuous TS-3 acceptance story proves lazy default-variant creation, required-signal completeness errors, exact range coverage, no-implicit-resampling horizon failures, stale-on-series-edit plus explicit revalidation, clone divergence, two runs with distinct variant/range/hash lineage, and the legacy raw-`ScenarioVersion` boundary. README now documents the TS-3 variant workflow and lineage boundary, `docs/series_tiempo/iter3/pruebas_manuales_ts3.md` adds the manual checklist, and this tracker records the focused acceptance command. Chrome-devtools MCP verification against the real PostgreSQL-backed app confirmed the same UI story in React, including stale/revalidate and lineage visibility. |
