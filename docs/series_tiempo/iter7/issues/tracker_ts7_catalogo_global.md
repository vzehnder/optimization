...
| TS7-022 | Cut Over To The Single Canonical Writer (C6) | AFK | ready-for-agent | In Review | TS7-009, TS7-018, TS7-021 | [TS7-022-cut-over-to-the-single-canonical-writer.md](TS7-022-cut-over-to-the-single-canonical-writer.md) |
| TS7-023 | Prove TS-7 End To End | AFK | ready-for-agent | Done | TS7-022 | [TS7-023-prove-ts7-end-to-end.md](TS7-023-prove-ts7-end-to-end.md) |
...
| --- | --- | --- | --- |
| 2026-09-07 | TS7-024 | Created -> In Review | Deleting a project now ends the retention of the TS-7 trail it owns, which chapter 9.6 had never contemplated: its restrictive foreign keys and append-only guards made `DELETE /api/projects/{id}` answer 500 on PostgreSQL for every project with canonical content, and the TS7-003 PostgreSQL teardown had been clearing `time_series_sets` by hand to work around it. `AnalystStore.delete_project` now declares itself in a purge scope, removes the closure in reverse dependency order through `app/time_series_purge.py` - ledgers, links, ingestions, object definitions, projection and canonical content - deletes the project and clears the declaration inside one transaction, so a failure restores the whole trail and leaves no declaration behind. The guards keep their names and codes and gain one condition: a declared purge may DELETE, while UPDATE stays refused with or without one, so sealed revisions, signal identities and the three ledgers remain unrewritable. Thirteen new tests pass on SQLite and two mirror the closure and the standing refusal on PostgreSQL inside a rolled-back transaction against the development database. The full Python suite passes with 1,249 tests. `docs/series_tiempo/iter7/decision_record_ts7_project_purge.md` records the exception to 9.6 and what it deliberately does not open. |
| 2026-09-06 | TS7-023 | Todo -> Done | `tests/test_ts7_acceptance.py` tells the eighteen observable stories once, over the same public surfaces the TS7-0xx suites use, and runs on SQLite and on PostgreSQL. The whole blocking matrix is green module by module on PostgreSQL across 23 modules; the five modules that fail on `energy_dispatch` fail on rows committed by earlier runs and by the TS7-021 manual pass, and each passes on a pristine database, so the matrix runs on `energy_dispatch_ts7_acceptance` and `docs/series_tiempo/iter7/acceptance_ts7.md` records why. That run found two real PostgreSQL-only defects and fixed both: the C4 identity sync resolved `case_time_series_bindings` as a canonical content table instead of a link-layer one, and three literal `LIKE 'canonical_binding:%'` patterns broke the psycopg placeholder parser. `test_ts2_acceptance` to `test_ts6_acceptance` pass unedited. AC-PER-01 to AC-PER-07 are all measured for the first time with their reference plans saved; TS7-005 had covered three. The manual Chrome pass with the real `.env` credentials ran the three complete flows with a clean console and found that the journey's `Crear especifica para este objeto` branch had no second step, so TS7-023 built that leg over the routes TS7-010 to TS7-012 already serve; no route and no generated API artifact changed. The full Python suite passes with 1.236 tests. `tsc`, `eslint`, 159 Vitest tests, `api:check` and the production build pass. The ledger reconstructs actor, role, reason, request and moment for every mutation and refuses deletion by API and below it. One limitation is recorded rather than fixed: `AnalystStore` cannot bootstrap an empty PostgreSQL database, because the base schema declares `hydraulic_time_series_sets` with a foreign key to `time_series_sets` before that table exists. |
| 2026-09-06 | TS7-022 | Todo -> In Review | C6 now rechecks the live C5/C4/projection/journal gate under pause and durably flips canonical reads and the sole canonical writer. Code guards plus SQLite triggers and PostgreSQL trigger/permission DDL make the complete legacy content closure read-only; no C7 contraction occurs. Current TS-2 reads retain their JSON shape as marked private aliases, while manual edits, replacement uploads and links delegate to idempotent canonical publications and generic bindings. The TS-5 hydraulic adapter remains visible and migrates on demand into a canonical set without rewriting its source. Regular internal identities receive the canonical UI flag only after C6 and external identities remain refused. Seven zero-tolerance triggers, three measured thresholds, the 72-hour observation window and 30-day daily reconciliation are executable; any trigger pauses mutations, keeps reads live and requires roll-forward. Eleven focused tests, 142 directed regressions, the full Python suite (1,214 tests), generated OpenAPI checks, TypeScript, ESLint, 158 Vitest tests and the production build pass. PostgreSQL execution remains opt-in because its test URL was unavailable; the generated trigger/revoke contract is covered. |
